    ...
    int n )
{
    ...
    // Query workspace needed for Cholesky factorization
    int lwork = 0;
    CUSOLVER_CHECK(
    ...
    for (int k = 0; k < n; k += r) 
    {
        ...
        // -----------------------------------------------
        float* d_panel = d_A + (k * ld + k);  // **Row-major offset calculation**

        // **cuSOLVER Cholesky: Now using LOWER**
        CUSOLVER_CHECK(
            cusolverDnSpotrf(
                cusolverH,
                CUBLAS_FILL_MODE_LOWER,  // **Fix for row-major**
                r_block,
                d_panel,
                ld,
                panel_workspace,
        ...
        if (sub > 0) {
            ...
            // **Row-major fix: Use LEFT instead of RIGHT**
            CUBLAS_CHECK(
                cublasStrsm(handle,
                CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER,
                CUBLAS_OP_T, CUBLAS_DIAG_NON_UNIT,
            ...
            // -----------------------------------------------
            // 3) GEMM-like update (SYRK) A -= B * B^T
            // -----------------------------------------------
            cudaStreamWaitEvent(gemmStream, trsm_done, 0);
...
int uniform_prec_fused_cholesky_cutlass(   float* d_A,         // row-major NxN matrix on device
    int ld,             // leading dimension = number of columns = N
    int r,              // block size
    float* diag_A,      // (not used in this snippet)
    float* updated_diag,// (not used in this snippet)
